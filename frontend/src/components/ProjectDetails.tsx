...
const ProjectDetails: FC<IProps> = ({ project }) => {
  const { projectDetails, toggleProjectDetails, language, darkMode } = usePreferenceStatus();
  ...
  return (
    <div className="modal">
      ...
      ></div>
      <div className="modal--content border-4 right-0 left-0 top-0 bottom-0 w-11/12 md:w-2/3 inline-table m-auto bg-bgColor dark:bg-darkModeBgColor dark:text-bgColor border-black dark:border-bgColor">
        {/* Content */}
        <div className="flex flex-col justify-between relative h-full">
          ...
          <div className="flex justify-end my-1 pr-1">
            {darkMode === "true" ? (
              <img
                ...
                onClick={() => closeModal(project.id)}
                className="w-5 cursor-pointer pr-1 my-1"
              />
            ) : (
              <img
                src={cross}
          ...
          <div className="flex flex-col h-full justify-between">
            ...
            <div className="flex px-4 mb-5">
              <div className="flex bg-black dark:bg-bgColor relative px-4 py-1 group">
                <a
                  href={project.code}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-white dark:text-black text-lg transition duration-300 ease-linear group-hover:-translate-x-1"
                >
                  Code
                </a>
                {darkMode === "true" ? (
              ...
              {project.url === "" ? (
                ...
                <div className="flex bg-black dark:bg-bgColor relative px-4 py-1 group ml-4">
                  <a
                    href={project.url}
                    target="_blank"
                    rel="noopener noreferrer"
